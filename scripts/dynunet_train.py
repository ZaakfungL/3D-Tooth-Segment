...
from src.models.dynunet import DynUNet3D
...
def train_dynunet(config):
    ...
    # ================= 3. 模型与优化器 =================
    model = DynUNet3D(in_channels=1, out_channels=2).to(device)
    ...
    print(f"\n{'='*20} 开始训练 DynUNet (基于Iteration) {'='*20}")
...
if __name__ == "__main__":
    parser = get_config_argument_parser(description="DynUNet 训练脚本")
    parser.add_argument("--seed", type=int, default=2025, help="随机种子 (默认: 2025)")
    args = parser.parse_args()

    default_config_path = os.path.join(project_root, "configs", "dynunet_train.yaml")
