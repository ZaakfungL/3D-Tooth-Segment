...
from src.models.segresnet import SegResNet3D
...
def train_segresnet(config):
    ...
    # ================= 3. 模型与优化器 =================
    model = SegResNet3D(in_channels=1, out_channels=2).to(device)
    ...
    print(f"\n{'='*20} 开始训练 SegResNet (基于Iteration) {'='*20}")
    ...
    while iteration < max_iterations:
        ...
        if iteration % val_interval == 0:
            del inputs,outputs,  labels_batch, loss
...
if __name__ == "__main__":
    parser = get_config_argument_parser(description="SegResNet 训练脚本")
    parser.add_argument("--seed", type=int, default=2025, help="随机种子 (默认: 2025)")
    args = parser.parse_args()

    default_config_path = os.path.join(project_root, "configs", "segresnet_train.yaml")
