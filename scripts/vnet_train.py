...
from src.models.vnet import VNet3D
...
def train_vnet(config):
    ...
    # ================= 3. 模型与优化器 =================
    model = VNet3D(in_channels=1, out_channels=2).to(device)
        ...
        if iteration % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                if metric > best_metric:
                    ...
                    save_path = os.path.join(model_save_dir, "best_vnet_model.pth")
...
if __name__ == "__main__":
    parser = get_config_argument_parser(description="VNet 训练脚本")
    parser.add_argument("--seed", type=int, default=2025, help="随机种子 (默认: 2025)")
    args = parser.parse_args()

    default_config_path = os.path.join(project_root, "configs", "vnet_train.yaml")
