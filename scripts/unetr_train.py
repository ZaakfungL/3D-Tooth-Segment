...
from src.models.unetr import UNETR3D
...
def train_unetr(config):
    ...
    # ================= 3. 模型与优化器 =================
    # UNETR 需要指定 img_size，必须与 roi_size 一致
    model = UNETR3D(
        in_channels=1,
        out_channels=2,
        img_size=roi_size
...
if __name__ == "__main__":
    parser = get_config_argument_parser(description="UNETR 训练脚本")
    parser.add_argument("--seed", type=int, default=2025, help="随机种子 (默认: 2025)")
    args = parser.parse_args()

    default_config_path = os.path.join(project_root, "configs", "unetr_train.yaml")
